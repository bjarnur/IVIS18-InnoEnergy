...
async function getConsumptionById(id){
  ...
  //function yrRange(yr){
    //return "timestamp >= '" + yr + "-01-01' AND timestamp <= '" + yr + "-12-31 *'";
  //}
  ...
  async function chooseYr(yr){
    var ret = {};
    var d;
    //TODO:-01-21 will be -12-31 afterwards
    for(let date = new Date(yr+"-01-01");date<=new Date(yr+"-01-21");date.setDate(date.getDate()+1)){
      d = date.toISOString().substr(0,10);
      ret[d] = (await generalSelect("SELECT SUM(value) as val FROM " + tables["consumption"].tableName + " WHERE id == '" + id +"'" + " AND timestamp LIKE '" + d + " %';"))[0].val;
    }
    return ret;
  }

  //get the range
  let timeBound = await generalSelect("SELECT MIN(timestamp) AS min, MAX(timestamp) AS max FROM " + tables["consumption"].tableName + " WHERE id == '" + id + "'");
  let yrBound = {};

  yrBound.min = parseInt(timeBound[0].min.slice(0,4));
  yrBound.max = parseInt(timeBound[0].max.slice(0,4));
  var ret = {};
  for(let yr = yrBound.min;yr<=yrBound.max;++yr){
    //ret[yr.toString()] = await generalSelect("SELECT value FROM " + tables["consumption"].tableName + " WHERE id == '" + id + "'" + " AND " + yrRange(yr) + ";");
...
async function getConsumptionByDate(id, fromDate, toDate) {
 return await generalSelect("SELECT timestamp, value FROM " + tables["consumption"].tableName + 
                            " WHERE id == '" + id + "'"  + 
