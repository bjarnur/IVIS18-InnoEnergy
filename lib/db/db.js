...
async function getConsumptionByDate(id, time, fromDate, toDate) {
  let res
  switch(time){
    case 'year':
      res = await getTimeGranularityOfIdData(id, "'%Y'", fromDate, toDate)
      break
    case 'month':
      res = await getTimeGranularityOfIdData(id, "'%Y-%m'", fromDate, toDate)
      break
    default:
      res = await getTimeGranularityOfIdData(id,"'%Y-%m-%d'", fromDate, toDate)
  }
  return time ? util.groupByTime(res, time) : res
}

async function getMonthlyMaxConsumption(id, fromDate, toDate, time) {
  let fuse = await getFuseByBuildingId(id)
  let res = await generalSelect("SELECT timestamp, value FROM " + tables["consumption"].tableName +
                            " WHERE id == '" + id + "'"  +
                            " AND timestamp >= '" + fromDate + "'" +
                            " AND timestamp <= '" + toDate + "';");
  return util.getMaxValuePerMonth(res, fuse)
}
...
async function getDailyMaxConsumption(id, fromDate, toDate, time) {
  ...
  let res = await generalSelect("SELECT timestamp, value FROM " + tables["consumption"].tableName +
                            ...
                            " AND timestamp >= '" + fromDate + "'" +
                            " AND timestamp <= '" + toDate + "';");
  return util.getMaxValuePerDay(res, fuse)
}

async function getFuseByBuildingId(id) {
  let res = await generalSelect("SELECT fuse FROM " + tables["building_info"].tableName +
                            " WHERE id == '" + id + "';");
  console.log(res[0].fuse)
  return constants.FUSES[res[0].fuse];
}

async function getTimeGranularityOfIdData(id, strftime, fromDate, toDate){
  if(fromDate && toDate){
    return await generalSelect("SELECT strftime(" + strftime + ", timestamp) as time, " +
                              "SUM(value) as sum, " +
                              "COUNT(value) as count FROM " + tables["consumption"].tableName +
                              " WHERE id == '" + id + "'" +
                              " AND timestamp >= '" + fromDate + "'" +
                              " AND timestamp <= '" + toDate + "' GROUP BY time;")
  } else {
    return await generalSelect("SELECT strftime(" + strftime + ", timestamp) as time, " +
                              "SUM(value) as sum, " +
                              "COUNT(value) as count FROM " + tables["consumption"].tableName +
                              " WHERE id == '" + id + "' GROUP BY time;")
  }
}
