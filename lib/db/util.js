...
function addFuseCapacity(result, fuse) {
  //console.log(result)
  let capacity = {};  
  ...
  for(let key in result) {
    for(let i in result[key].values) {
...
function getMaxValuePerMonth(result, fuse) {
  ...
  for(let i in result) {
    ...
    let curr_val = ins["values"][month] ? parseFloat(ins["values"][month]) : 0
    let new_val = parseFloat(result[i].value)    
    let sumSoFar = parseFloat(ins["sum"])
...
function getMaxValuePerDay(result, fuse) {
  ...
  for(let i in result) {
    ...
    let curr_val = ins["values"][day] ? parseFloat(ins["values"][day]) : 0
    let new_val = parseFloat(result[i].value)
    let sumSoFar = parseFloat(ins["sum"])
    let countSoFar = parseInt(ins["count"])
